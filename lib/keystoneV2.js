...
function Keystonev2(endpoint_url)
{
...
Keystonev2.prototype.setTimeout = function(new_timeout)
{
  this.timeout = new_timeout;
};

Keystonev2.prototype.setRequestID = function(request_id)
{
  this.request_id = request_id;
};  

Keystonev2.prototype.setUserName = function(user_name)
{
  this.user_name = user_name;
};

Keystonev2.prototype.setLogger = function(logger)
{
  this.logger = logger;
};

//this should only be used for dependency injection
Keystonev2.prototype.setRequest = function(request_lib)
{
...
Keystonev2.prototype.setMangler = function(mangle_lib)
{
...
//NOTE: because Keystonev2 is non-project specific this function is different than all the other classes with it
Keystonev2.prototype.getRequestOptions = function(auth_token, path, json_value)
{
  ...
  if(!request_timeout)
  {
    ...
    request_timeout = Keystonev2.timeout;
  }
  var return_object = {
    uri: this.url + path,
...
//authorizes the users against the specified Keystonev2
...
Keystonev2.prototype.getToken = function(username, password, cb)
{
  var self = this;
  var auth_data = {
    'auth': {
  ...
  var request_options = this.getRequestOptions('bogus', '/v2.0/tokens', auth_data);
  ...
  this.request.post(request_options, function(error, response, body){
    ...
    //now we good
    cb(null, self.mangleObject('Token', body));
  });
};
...
Keystonev2.prototype.getProjectTokenForReal = function(auth_data, cb)
{
  var self = this;
  ...
  var request_options = this.getRequestOptions('bogus', '/tokens', auth_data);
...
//make a callback(error, project_authorization) with all of the data on a project and an access token for further calls on it
//NOTE: this is not the admin function that gets project details - you have to do this so I'm not bothering with that
Keystonev2.prototype.getProjectToken = function(access_token, project_id, cb)
{
  var auth_data = {
    auth:{
      token : {
...
Keystonev2.prototype.getProjectTokenById = Keystonev2.prototype.getProjectToken;



//make a callback(error, project_authorization) with all of the data on a project and an access token for further calls on it
//NOTE: this is not the admin function that gets project details - you have to do this so I'm not bothering with that
Keystonev2.prototype.getProjectTokenByName = function(access_token, domain_id, project_name, cb)
{
  var auth_data = {
    auth:{
      identity:{
...
Keystonev2.prototype.listProjects = function(admin_access_token, cb)
{
  var self = this;
  var request_options = this.getRequestOptions(admin_access_token, '/v2.0/tenants', true);
  ...
  this.request.get(request_options, function(error, response, body){
    ...
    for(var n = 0; n < body.tenants.length; n++)
    {
      projects_array[n] = self.mangleObject('Project', body.tenants[n]);
    }
...
Keystonev2.prototype.listEndpoints = function(admin_access_token, cb)
{
  var self = this;
  var request_options = this.getRequestOptions(admin_access_token, '/v2.0/endpoints', true);
  request_options.metricPath = 'remote-calls.keystone.projects.list';
  request_options.validateStatus = true;
  request_options.requireBodyObject = 'projects';
...
Keystonev2.prototype.listUserProjects = function(username, access_token, cb)
{
  var self = this;
...
Keystonev2.prototype.getProjectByName = function(admin_access_token, project_name, cb)
{
  var self = this;
...
Keystonev2.prototype.listRoles = function(project_token, cb)
{
  var self = this;
...
Keystonev2.prototype.listRoleAssignments = function(project_token, project_id, cb)
{
  var self = this;
...
//NOTE: this is only works if the user is authed as an admin or projectAdmin
Keystonev2.prototype.addRoleAssignment = function(project_token, project_id, entry_id, entry_type, role_id, cb)
{
  var request_options = {};
...
//NOTE: this is only works if the user is authed as an admin or projectAdmin
Keystonev2.prototype.removeRoleAssignment = function(project_token, project_id, entry_id, entry_type, role_id, cb)
{
  var request_options = {};
  var entry_type_path = 'users';
...
Keystonev2.prototype.listMetaEnvironments = function(auth_token, cb)
{
  var self = this;
  var request_options = {};
...
Keystonev2.prototype.listMetaOwningGroups = function(auth_token, cb)
{
  var self = this;
  var request_options = {};
...
Keystonev2.prototype.listProjectMeta = function(project_token, project_id, cb)
{
  var self = this;
  var request_options = {};
  var meta_object = {};
...
//calls back with cb(error, meta_object)
Keystonev2.prototype.updateProjectMeta = function(project_token, project_id, new_meta, cb)
{
  var self = this;
  var request_options = {};
